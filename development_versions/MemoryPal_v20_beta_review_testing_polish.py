...
VERSION = "v20 Beta"
TITLE = "Review testing and Q/A polish"
ACCENT = "#34c759"
FEATURES = [
    "Question cards can be saved with or without a saved answer.",
    "A separate Test Lab page lets the user test and return.",
    "Smart Check highlights the suggested bucket visually.",
    "Mini-story now auto-generates ordered memory scenes.",
]
...
class StageApp(tk.Tk):
    ...
    def build(self):
        ...
        test = ttk.Frame(page, style="Alt.TFrame", padding=self.px(22))
        test.pack(fill="x", pady=(self.px(18), self.px(14)))
        ttk.Label(test, text="Test Lab", style="Alt.TLabel").pack(anchor="w")
        ttk.Label(test, text="Smart Check: Close enough | Bucket: Good", style="Alt.TLabel").pack(anchor="w", pady=(self.px(8), self.px(12)))
        row = tk.Frame(test, bg="#eef5ff")
        row.pack(fill="x")
        for label, color in [("Again", "#ffe8e6"), ("Review", "#fff3d6"), ("Good", "#e8f8ef"), ("Easy", "#e7f0ff")]:
            tk.Label(row, text=label, bg=color, fg="#111827", padx=self.px(14), pady=self.px(8)).pack(side="left", padx=(0, self.px(8)))
